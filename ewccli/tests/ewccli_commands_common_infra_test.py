...
def test_resolve_cpu_defaults(conn, backend):
    code, msg, result = resolve_image_and_flavor(
        conn, backend, federee="EUMETSAT", is_gpu=False
    )
    assert code == 0
...
def test_resolve_eumetsat_gpu_defaults(conn, backend):
    code, msg, result = resolve_image_and_flavor(
        conn, backend, federee="EUMETSAT", is_gpu=True
    )
    assert code == 0
    assert (
        result["normalized_image_name"]
        == ewc_hub_config.EWC_CLI_OS_GPU_IMAGES_SITE_MAP["EUMETSAT"]
    )
...
def test_resolve_ecmwf_gpu_defaults(conn, backend):
    code, msg, result = resolve_image_and_flavor(
        conn, backend, federee="ECMWF", is_gpu=True
    )
    assert code == 0
    assert (
        result["normalized_image_name"]
        == ewc_hub_config.EWC_CLI_OS_GPU_IMAGES_SITE_MAP["ECMWF"]
    )
...
def test_resolve_unknown_image(conn, backend):
    code, msg, result = resolve_image_and_flavor(
        conn, backend, federee="EUMETSAT", image_name="Unknown-OS-1234"
    )
    assert code == 1
...
def test_post_deploy_server_setup_missing_ip(conn):
    ...
    with patch(
        "ewccli.commands.commons_infra.resolve_machine_ip", return_value=(0, "ok", None)
    ):
        server_inputs = {
            "server_name": "vm1",
            "external_ip": False,
...
def test_create_server_command_success(conn):
    ...
    deploy_outputs = {"server_info": {"id": "123", "name": "vm1"}}
